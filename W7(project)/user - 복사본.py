...
class User:
    ...
    def execute(self, other, grave, ext=None):
        ...
        if self.selectedCard.type == 1:
            ...
            #대상이 카드를 버리고 새로 뽑습니다.
            note1 = self.nick+"님이 5번카드 대상으로 "+other.nick+"님을 지정하였습니다."#모두
            note.append(note1)
            note2 = "현재의 카드를 버리고 새로운 카드를 한장 뽑으세요."#대상
            note.append(note2)
            if other.prossessionCard[0].type == 8:
                other.isAlive= False
                grave.append([other.prossessionCard[0],-1])
                note3 = other.nick"님의 8번 카드가 공개되며 운명하셨습니다."#모두
            else:
                grave.append([other.prossessionCard[0],-1])
                other.prossessionCard.append(choice(ext))
                ext.remove(other.prossessionCard[0])
                note3 = other.nick+"님이 새 카드를 뽑으셨습니다."#모두
            note.append(note3)
            ...
            note1 = self.nick+"님이 6번카드 대상으로 "+other.nick+"님을 지정하였습니다."#모두
            note.append(note1)
            note2 = self.nick+"님과 카드를 교환하세요."#대상
            note.append(note2)
            return note
        elif self.selectedCard.type == 7:
            note1 = self.nick+"님이 7번 카드를 선택하셨습니다."#모두
            note.append(note1)
            return note